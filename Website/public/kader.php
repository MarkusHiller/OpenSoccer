    ...
    if (isset($_POST['positionToSearch'])) {
        ...
        if ($positionToSearch == 'T' OR $positionToSearch == 'A' OR $positionToSearch == 'M' OR $positionToSearch == 'S') {
            ...
            switch ($positionToSearch) {
                case 'T': $whatIsSearched = _('Torhüter');
                    break;
                case 'A': $whatIsSearched = _('Abwehrspieler');
                    setTaskDone('instruct_youthcoach');
                    break;
                case 'M': $whatIsSearched = _('Mittelfeldspieler');
                    break;
                case 'S': $whatIsSearched = _('Stürmer');
                    break;
            }
        ...
        <table>
            <thead>
                <tr class="odd">
                    ...
                    <th scope="col"><a href="/kader.php?orderBy=AL"><?php echo _('AL'); ?></a></th>
                    <th scope="col"><a href="/kader.php?orderBy=staerke"><?php echo _('Stärke'); ?></a></th>
                    <th scope="col"><a href="/kader.php?orderBy=PS"><?php echo _('PS'); ?></a></th>
                    <th scope="col"><a href="/kader.php?orderBy=MW"><?php echo _('MW'); ?></a></th>
                </tr>
            </thead>
            <tbody>
                <?php
...
// LIGA-WERT UND MORAL DER SPIELER AKTUALISIEREN ENDE
                ...
                while ($sql3 = mysql_fetch_assoc($sql2)) {
                    ...
                    if ($sql3['team'] != $cookie_team OR $sql3['leiher'] != 'keiner') {
                        $trCSS .= 'verliehen';
                    }
                    $trCSS = trim($trCSS);
                    echo '<tr';
...
// BESETZUNGEN BERECHNEN ANFANG
            ...
            while ($posSel3 = mysql_fetch_assoc($posSel2)) {
                switch ($posSel3['position']) {
                    case 'T': $posArr['T'] = $posSel3['anzahl'] / 1.5;
                        break;
                    case 'A': $posArr['A'] = $posSel3['anzahl'] / 6;
                        break;
                    case 'M': $posArr['M'] = $posSel3['anzahl'] / 6;
                        break;
                    case 'S': $posArr['S'] = $posSel3['anzahl'] / 3;
                        break;
                }
            }
...
// BEWERTUNG AUSGEBEN ANFANG
            function besetzungToWort($besetzung) {
                if ($besetzung < 0.5) {
                    return _('sehr schlecht');
                } elseif ($besetzung < 0.8) {
                    return _('schlecht');
                } elseif ($besetzung < 1.1) {
                    return _('solide');
                } elseif ($besetzung < 1.4) {
                    return _('gut');
                } elseif ($besetzung < 1.7) {
                    return _('sehr gut');
                } elseif ($besetzung < 2) {
                    return _('hervorragend');
                } elseif ($besetzung < 2.3) {
                    return _('weltklasse');
                } else {
                    return _('überbesetzt');
                }
            }
            ...
            foreach ($posArr as $position => $besetzung) {
                switch ($position) {
                    case 'T': $posToSearch = _('Torwart');
                        $anzahlSpieler = $besetzung * 1.5;
                        break;
                    case 'A': $posToSearch = _('Abwehr');
                        $anzahlSpieler = $besetzung * 6;
                        break;
                    case 'M': $posToSearch = _('Mittelfeld');
                        $anzahlSpieler = $besetzung * 6;
                        break;
                    case 'S': $posToSearch = _('Sturm');
                        $anzahlSpieler = $besetzung * 3;
                        break;
                    default: $posToSearch = '?';
                        $anzahlSpieler = 0;
                        break;
...
// FESTLEGEN WAS GESUCHT WERDEN SOLL ANFANG
    ...
    echo '<option value="T"';
    if ($currentlySearching == 'T') {
        echo ' selected="selected"';
    } echo '>' . _('Torwart suchen') . '</option>';
    echo '<option value="A"';
    if ($currentlySearching == 'A') {
        echo ' selected="selected"';
    } echo '>' . _('Abwehr suchen') . '</option>';
    echo '<option value="M"';
    if ($currentlySearching == 'M') {
        echo ' selected="selected"';
    } echo '>' . _('Mittelfeld suchen') . '</option>';
    echo '<option value="S"';
    if ($currentlySearching == 'S') {
        echo ' selected="selected"';
    } echo '>' . _('Sturm suchen') . '</option>';
