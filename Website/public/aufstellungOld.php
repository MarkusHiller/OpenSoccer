...
foreach ($spieltypenMoeglich as $spieltypSingle) {
    echo '<a href="/aufstellung.php?spieltypAufstellung=' . $spieltypSingle . '" class="pagenava';
    if ($spieltypAufstellung == $spieltypSingle) {
        echo ' aktiv';
    } echo '">' . $spieltypSingle . '</a>&nbsp;';
}
echo '</p>';
// SPIELTYP ENDE
?>
...
// SPIELER IN ARRAY LESEN ANFANG
        ...
        while ($player3 = mysql_fetch_assoc($player2)) {
            if ($spieltypAufstellung == 'Pokal' && $player3['pokalNurFuer'] != '' && $player3['pokalNurFuer'] != $cookie_team) {
                continue;
            }
            $player_name = mb_substr($player3['vorname'], 0, 1, 'utf-8') . '. ' . $player3['nachname'] . ' (' . number_format($player3['staerke'], 1, ',', '.') . ')';
            switch ($player3['position']) {
                case 'T': $player_t[] = array($player3['ids'], $player_name, $player3['startelfWert']);
                    break;
                case 'A': $player_a[] = array($player3['ids'], $player_name, $player3['startelfWert']);
                    break;
                case 'M': $player_m[] = array($player3['ids'], $player_name, $player3['startelfWert']);
                    break;
                case 'S': $player_s[] = array($player3['ids'], $player_name, $player3['startelfWert']);
                    break;
            }
        }
...
// SPIELER-MARKIERUNGEN ENDE
        ...
        <form action="/aufstellung.php?spieltypAufstellung=<?php echo $spieltypAufstellung; ?>" method="post" accept-charset="utf-8">
            ...
            <div class="os-lineup-fieldm">
                <div class="visible-mobile"><p><strong><?php echo _('Sturm:'); ?></strong></p></div>
                <select name="2" size="1" class="os-lineup-playerm">
                    <?php
                    foreach ($player_s as $player_ss) {
                        echo '<option value="' . $player_ss[0] . '"';
                        if ($player_ss[2] == 2) {
                            echo ' selected="selected"';
                        } echo '>' . $player_ss[1] . '</option>';
                    }
                    ?>
                </select>
                <select name="1" size="1" class="os-lineup-playerm">
                    <?php
                    foreach ($player_s as $player_ss) {
                        echo '<option value="' . $player_ss[0] . '"';
                        if ($player_ss[2] == 1) {
                ...
                <select name="6" size="1" class="os-lineup-playerm">
                    <?php
                    foreach ($player_m as $player_ms) {
                        echo '<option value="' . $player_ms[0] . '"';
                        if ($player_ms[2] == 6) {
                            echo ' selected="selected"';
                        } echo '>' . $player_ms[1] . '</option>';
                    }
                    ?>
                </select>
                <select name="5" size="1" class="os-lineup-playerm">
                    <?php
                    foreach ($player_m as $player_ms) {
                        echo '<option value="' . $player_ms[0] . '"';
                        if ($player_ms[2] == 5) {
                ...
                <select name="4" size="1" class="os-lineup-playerm">
        <?php
        foreach ($player_m as $player_ms) {
            echo '<option value="' . $player_ms[0] . '"';
            if ($player_ms[2] == 4) {
        ...
        ?>
                ...
                <select name="3" size="1" class="os-lineup-playerm">
                    <?php
                    foreach ($player_m as $player_ms) {
                        echo '<option value="' . $player_ms[0] . '"';
                        if ($player_ms[2] == 3) {
                ...
                <select name="10" size="1" class="os-lineup-playerm">
        <?php
        foreach ($player_a as $player_as) {
            echo '<option value="' . $player_as[0] . '"';
            if ($player_as[2] == 10) {
                echo ' selected="selected"';
            } echo '>' . $player_as[1] . '</option>';
        }
        ?>
                </select>
                <select name="9" size="1" class="os-lineup-playerm">
        <?php
        foreach ($player_a as $player_as) {
            echo '<option value="' . $player_as[0] . '"';
            if ($player_as[2] == 9) {
        ...
        ?>
                ...
                <select name="8" size="1" class="os-lineup-playerm">
        <?php
        foreach ($player_a as $player_as) {
            echo '<option value="' . $player_as[0] . '"';
            if ($player_as[2] == 8) {
        ...
        ?>
                ...
                <select name="7" size="1" class="os-lineup-playerm">
                    <?php
                    foreach ($player_a as $player_as) {
                        echo '<option value="' . $player_as[0] . '"';
                        if ($player_as[2] == 7) {
                ...
                <select name="11" size="1" class="os-lineup-playerm">
        <?php
        foreach ($player_t as $player_ts) {
            echo '<option value="' . $player_ts[0] . '"';
            if ($player_ts[2] == 11) {
                echo ' selected="selected"';
            } echo '>' . $player_ts[1] . '</option>';
        }
        ?>
                </select>
            </div>
        ...
        <table>
            <thead>
                <tr class="odd">
                    ...
                    <th scope="col"><a href="/aufstellung.php?orderBy=AL"><?php echo _('AL'); ?></a></th>
                    <th scope="col"><a href="/aufstellung.php?orderBy=staerke"><?php echo _('Stärke'); ?></a></th>
                    <th scope="col"><a href="/aufstellung.php?orderBy=FR"><?php echo _('FR'); ?></a></th>
                    <th scope="col"><a href="/aufstellung.php?orderBy=MO"><?php echo _('MO'); ?></a></th>
                </tr>
            </thead>
            <tbody>
                <?php
                ...
                $sql1 = "SELECT ids, position, vorname, nachname, wiealt, moral, staerke, talent, frische, startelf_" . $spieltypAufstellung . " AS startelfWert, verletzung FROM " . $prefix . "spieler WHERE team = '" . $cookie_team . "' ORDER BY position = 'S', position = 'M', position = 'A', position = 'T', " . $orderBy;
                ...
                while ($sql3 = mysql_fetch_assoc($sql2)) {
                    ...
                    if (isset($markierungen[$sql3['ids']])) {
                        switch ($markierungen[$sql3['ids']]) {
                            case 'Blau': $farbcode = ' style="background-color:#00f"';
                                break;
                            case 'Gelb': $farbcode = ' style="background-color:#ff0"';
                                break;
                            case 'Rot': $farbcode = ' style="background-color:#f00"';
                                break;
                            case 'Gruen': $farbcode = ' style="background-color:#0f0"';
                                break;
                            case 'Pink': $farbcode = ' style="background-color:#f0f"';
                                break;
                            case 'Aqua': $farbcode = ' style="background-color:#0ff"';
                                break;
                            case 'Silber': $farbcode = ' style="background-color:#c0c0c0"';
                                break;
                            case 'Lila': $farbcode = ' style="background-color:#800080"';
                                break;
                            case 'Oliv': $farbcode = ' style="background-color:#808000"';
                                break;
                            default: $farbcode = '';
                                break;
                        }
                    ...
                    if ($counter % 2 == 0) {
                        if ($sql3['verletzung'] != 0) {
                            echo '<tr class="os-player-row-injured">';
                        } else {
                            echo '<tr>';
                        }
                    } else {
                        if ($sql3['verletzung'] != 0) {
                            echo '<tr class="odd os-player-row-injured">';
                        } else {
                            echo '<tr class="odd">';
                        }
                    }
