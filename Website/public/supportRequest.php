...
switch ($sql3['open']) {
    case 1: $entryNumber .= _('offen');
        break;
    case 0: $entryNumber .= _('umgesetzt');
        break;
    case -1: if ($sql3['category'] == 'Vorschlag') {
            $entryNumber .= _('abgelehnt');
        } else {
            $entryNumber .= _('geklärt');
        } break;
    default: exit;
        break;
}
...
// ANFRAGE ALS GELESEN MARKIEREN ENDE
    ...
    <p style="text-align:right">
        ...
        <?php if (($_SESSION['status'] == 'Admin' OR $_SESSION['status'] == 'Helfer') && $sql3['open'] == 1 && $sql3['visibilityLevel'] == 0) {
            echo ' <a href="/support.php?del=' . id2secure($requestID) . '" class="pagenava" onclick="return confirm(\'' . _('Bist Du sicher?') . '\')">' . _('Anfrage löschen') . '</a>';
        } ?>
    <?php
    if (($_SESSION['status'] == 'Admin' || $_SESSION['status'] == 'Helfer') && $sql3['open'] == 1) {
