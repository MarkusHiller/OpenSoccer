...
try {
	...
	Log::logToFile("debug_cup_auslosen", "Land: " . $landToSimulate4);
	...
	if (date('H', time()) == 10 OR date('H', time()) == 11) { 
		Log::logToFile("debug_cup_auslosen", "Exit time: " . date('H', time()));
		exit; 
	}
	...
	Log::logToFile("debug_cup_auslosen", "Land Count: " . var_export($landCnt, true));

	// wenn noch ausstehende Spiele da sind, abbrechen
	if ($landCnt[0] > 0) { 
		Log::logToFile("debug_cup_auslosen", "Exit landCnt");
		exit; 
	}
	...
	Log::logToFile("debug_cup_auslosen", "Next round: " . $naechste_runde);
	// WELCHE RUNDE ENDE
	...
	if ($naechste_runde == 1) {
		...
		Log::logToFile("debug_cup_auslosen", "Teams: " . var_export($teams2, true));	
		...
		while ($teams3 = mysql_fetch_assoc($teams2)) {
			...
			if (intval($tore[0]) > intval($tore[1])) {
				...
				Log::logToFile("debug_cup_auslosen", "Exit results: " . var_export($teams3, true));	
				exit;
			}
		}
		...
		if ($naechste_runde == 2) {
			...
			Log::logToFile("debug_cup_auslosen", "Freilose: " . var_dump($freiLosVerteilung2));	
	...
	Log::logExceptionToFile("cron_cup_auslosen", $e);
}
?>