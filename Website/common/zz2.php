            ...
            <ul id="nav">
                <?php if ($loggedin == 0) { ?>
                    <li<?php
                        ...
                        ?>><a href="/"><?php echo _('Startseite'); ?></a></li>
                    <li<?php
                        if ($_SERVER['SCRIPT_NAME'] == '/tour.php') {
                            echo ' id="current"';
                        }
                        ?>><a href="#" onclick="document.getElementById('lusername').value = 'Demo'; document.getElementById('lpassword').value = 'demo'; document.getElementById('login_form').submit(); return false;">Demo-Account</a></li>
                    <?php } else { ?>
                        <?php
...
// PN-ANZAHL ANFANG
                        ...
                        if (!isset($_SESSION['last_pn_anzahl'])) {
                            $_SESSION['last_pn_anzahl'] = 0;
                        }
                        if ($_SESSION['last_pn_anzahl'] < 0) {
                            $_SESSION['last_pn_anzahl'] = 0;
                        }
...
// FREUNDE-ANZAHL ANFANG
                        if (!isset($_SESSION['last_freunde_check'])) {
                            $_SESSION['last_freunde_check'] = 0;
                        }
                        if (!isset($_SESSION['last_freunde_anzahl'])) {
                            $_SESSION['last_freunde_anzahl'] = 0;
                        }
                        if ($_SESSION['last_freunde_anzahl'] < 0) {
                            $_SESSION['last_freunde_anzahl'] = 0;
                        }
...
// LEIHGABEN-ANGEBOTE ANFANG
                        if (!isset($_SESSION['last_leihgaben_check'])) {
                            $_SESSION['last_leihgaben_check'] = 0;
                        }
                        if (!isset($_SESSION['last_leihgaben_anzahl'])) {
                            $_SESSION['last_leihgaben_anzahl'] = 0;
                        }
                        if ($_SESSION['last_leihgaben_anzahl'] < 0) {
                            $_SESSION['last_leihgaben_anzahl'] = 0;
                        }
...
// TESTSPIEL-ANFRAGEN ANFANG
                        if (!isset($_SESSION['last_testspiele_check'])) {
                            $_SESSION['last_testspiele_check'] = 0;
                        }
                        if (!isset($_SESSION['last_testspiele_anzahl'])) {
                            $_SESSION['last_testspiele_anzahl'] = 0;
                        }
                        if ($_SESSION['last_testspiele_anzahl'] < 0) {
                            $_SESSION['last_testspiele_anzahl'] = 0;
                        }
                        if ($cookie_team != '__' . $cookie_id) {
...
// CHATTER ONLINE ANFANG
                        if (!isset($_SESSION['last_chatter_check'])) {
                            $_SESSION['last_chatter_check'] = 0;
                        }
                        if (!isset($_SESSION['last_chatter_anzahl'])) {
                            $_SESSION['last_chatter_anzahl'] = 0;
                        }
                        if ($_SESSION['last_chatter_anzahl'] < 0) {
                            $_SESSION['last_chatter_anzahl'] = 0;
                        }
...
// CHATTER ONLINE ENDE
    ...
    <?php if ($cookie_team != '__' . $cookie_id) { ?><li><a href="/protokoll.php"><?php echo _('Protokoll'); ?></a></li><?php } ?>
                    ...
                    <li class="menueintrag"<?php
    if (substr($_SERVER['SCRIPT_NAME'], 1, 5) == 'stat_' OR $_SERVER['SCRIPT_NAME'] == '/top_manager.php' OR $_SERVER['SCRIPT_NAME'] == '/manager_der_saison.php') {
        echo ' id="current"';
    }
    ?>><a href="/top_manager.php"><?php echo _('Ranking'); ?></a>
    <?php if ($cookie_team != '__' . $cookie_id) { ?>
                    ...
                    <li class="menueintrag"<?php
                        if ($_SERVER['SCRIPT_NAME'] == '/transfermarkt.php' OR $_SERVER['SCRIPT_NAME'] == '/marktschreier.php' OR $_SERVER['SCRIPT_NAME'] == '/transfermarkt_leihe.php' OR $_SERVER['SCRIPT_NAME'] == '/lig_transfers.php' OR $_SERVER['SCRIPT_NAME'] == '/beobachtung.php' OR $_SERVER['SCRIPT_NAME'] == '/transferliste.php') {
                            echo ' id="current"';
                        }
    ...
    <?php if ($cookie_team != '__' . $cookie_id) { ?><li><a href="/beobachtung.php"><?php echo _('Beobachtung'); ?></a></li><?php } ?>
                        ...
                        <?php if ($cookie_team != '__' . $cookie_id) { ?><li class="menueintrag"<?php
                    if ($_SERVER['SCRIPT_NAME'] == '/kader.php' OR $_SERVER['SCRIPT_NAME'] == '/aufstellung.php' OR $_SERVER['SCRIPT_NAME'] == '/taktik.php' OR $_SERVER['SCRIPT_NAME'] == '/vertraege.php' OR $_SERVER['SCRIPT_NAME'] == '/kalender.php' OR $_SERVER['SCRIPT_NAME'] == '/entwicklung.php') {
                        echo ' id="current"';
                    }
                ...
                ?>><a href="/saison/lig_tabelle.php"><?php echo _('Saison'); ?></a>
                    ...
                    <li class="menueintrag"<?php
                if (substr($_SERVER['SCRIPT_NAME'], 0, 8) == '/support' OR $_SERVER['SCRIPT_NAME'] == '/tipps_des_tages.php' OR $_SERVER['SCRIPT_NAME'] == '/regeln.php' OR $_SERVER['REQUEST_URI'] == ('/post_schreiben.php?id=' . CONFIG_OFFICIAL_USER)) {
                    echo ' id="current"';
                }
                ?>><a href="/support.php"><?php echo _('Support') . " (" . SupportController::getUnreadedTicketCount($cookie_id, $_SESSION['status']) . ")"; ?></a>
                        <ul>
...
<?php } ?>
        ...
        <div id="content-wrap">
                <?php if ($loggedin == 1) { ?>
                <div id="sidebar-expander" onclick="(function (self) {
                                var sidebar = document.getElementById('sidebar');
                                self.style.display = 'none';
                                sidebar.style.display = 'block';
                            })(this);"><span><?php echo _('Seitenleiste ausklappen'); ?></span></div>
<?php } ?>
<?php if ($loggedin == 0) { ?>
...
<?php } ?>
                ...
                <div id="main<?php
if ($_SERVER['SCRIPT_NAME'] == '/amazonShop.php') {
    echo '_full';
}
?>">
