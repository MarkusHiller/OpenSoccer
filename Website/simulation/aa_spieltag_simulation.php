...
while ($sql3 = mysql_fetch_assoc($sql2)) {
    ...
    $fouls[$sql3['team2']] = 0;
    $gelbe_karten = array();
    $gelbe_karten[$sql3['team1']] = 0;
    $gelbe_karten[$sql3['team2']] = 0;
    $rote_karten = array();
    $rote_karten[$sql3['team1']] = 0;
    $rote_karten[$sql3['team2']] = 0;
    $abseits = array();
    $abseits[$sql3['team1']] = 0;
    $abseits[$sql3['team2']] = 0;
    $schuesse = array();
    $schuesse[$sql3['team1']] = 0;
    $schuesse[$sql3['team2']] = 0;
    ...
    switch ($sql3['typ']) {
        case 'Pokal': $temp_fanaufkommen += 30000;
            break;
        case 'Cup': $temp_fanaufkommen += 10000;
            break;
        case 'Liga': $temp_fanaufkommen += 15000;
            break;
        default: $temp_fanaufkommen += 5000;
            break;
    }
    ...
    while ($spieler_team1c = mysql_fetch_assoc($spieler_team1b)) {
        ...
        switch ($spieler_team1c['position']) {
            case 'T': continue 2;
            case 'A': $temp = round($spieler_team1c['staerke'] * 0.3);
                break;
            case 'M': $temp = round($spieler_team1c['staerke'] * 1.8);
                break;
            case 'S': $temp = round($spieler_team1c['staerke'] * 3.7);
                break;
        }
    ...
    if ($strafeWegenUnvollstaendigkeitTeam1 > 0) {
        $spielbericht .= ' [Sponsor-Strafe: ' . number_format($strafeWegenUnvollstaendigkeitTeam1, 0, ',', '.') . ' €]';
    }
    $spielbericht .= '</p>';
    ...
    while ($spieler_team2c = mysql_fetch_assoc($spieler_team2b)) {
        ...
        switch ($spieler_team2c['position']) {
            case 'T': continue 2;
            case 'A': $temp = round($spieler_team2c['staerke'] * 0.3);
                break;
            case 'M': $temp = round($spieler_team2c['staerke'] * 1.8);
                break;
            case 'S': $temp = round($spieler_team2c['staerke'] * 3.7);
                break;
        }
        $temp = mt_rand($temp, 45);
    ...
    $spielbericht .= substr($templist, 0, -2);
    if ($strafeWegenUnvollstaendigkeitTeam2 > 0) {
        $spielbericht .= ' [Sponsor-Strafe: ' . number_format($strafeWegenUnvollstaendigkeitTeam2, 0, ',', '.') . ' €]';
    }
    $spielbericht .= '</p>';
    ...
    $tore[$sql3['team1']] = 0;
    $tore[$sql3['team2']] = 0;
    if ($taktiken[$sql3['team1']][0] == 4) {
        $taktik1v = 70;
        $taktik1a = 125;
    } elseif ($taktiken[$sql3['team1']][0] == 3) {
        $taktik1v = 80;
        $taktik1a = 115;
    } elseif ($taktiken[$sql3['team1']][0] == 1) {
        $taktik1v = 115;
        $taktik1a = 80;
    } else {
        $taktik1v = 100;
    ...
    if ($taktiken[$sql3['team2']][0] == 4) {
        $taktik2v = 70;
        $taktik2a = 125;
    } elseif ($taktiken[$sql3['team2']][0] == 3) {
        $taktik2v = 90;
        $taktik2a = 110;
    } elseif ($taktiken[$sql3['team2']][0] == 1) {
        $taktik2v = 110;
        $taktik2a = 90;
    } else {
        $taktik2v = 100;
    ...
    if (isset($scorers1)) {
        if (is_array($scorers1)) {
            if (count($scorers1) > 3) {
                ...
                // TORE ENDE
                // VERLETZUNGEN ANFANG
                if (count($frischeWerteTeam1) == 0) {
                    $frischeAvgTeam1 = 0;
                } else {
                    $frischeAvgTeam1 = array_sum($frischeWerteTeam1) / count($frischeWerteTeam1);
                }
